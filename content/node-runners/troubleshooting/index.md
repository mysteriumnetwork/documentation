...
Providers can change the ordering of NAT traversal methods and prioritize NAT hole punching. In this case, ```bash --traversal="holepunching,manual,upnp"``` flag  should be added into the DAEMON_OPTS="" in the service configuration file: ```bash/etc/default/mysterium-node```. 
...
```bash --wireguard.listen.ports``` & ```bash --p2p.listen.ports```has been deprecated starting from the Testnet3+, use above flag to manually set range of listen ports.
